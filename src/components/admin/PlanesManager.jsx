...
export function PlanFormModal({ open, onClose, plan, planes, refresh }) {
  ...
  const guardar = async () => {
    ...
    // ✅ NUEVA LÓGICA
    if (plan?.id) {
      await db.update('rest:planes', plan.id, {
        nombre: form.nombre,
        almuerzos: Number(form.almuerzos),
        dias: Number(form.dias),
        precio: Number(form.precio),
        activo: form.activo,
      });
    } else {
      await db.insert('rest:planes', {
        nombre: form.nombre,
        almuerzos: Number(form.almuerzos),
        dias: Number(form.dias),
        precio: Number(form.precio),
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          <Input
            label="Almuerzos"
            type="number"
            value={form.almuerzos}
            onChange={(v) => setForm({ ...form, almuerzos: +v })}
            error={errors.almuerzos}
          />
          <Input
            label="Días"
            type="number"
        ...
        <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: T.bg }}>
          <div>
            ...
            <p className="text-xs" style={{ color: T.textSoft }}>
              Los planes inactivos no se muestran a clientes
            </p>
          </div>
          <Toggle
            checked={form.activo}
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Btn onClick={guardar}>
            {plan?.id ? 'Guardar cambios' : 'Crear plan'}
          </Btn>
        </div>
      </div>
    </Modal>
...
export function PlanesTab({ planes, refresh }) {
  ...
  const eliminar = async (id) => {
    ...
    await db.update('rest:planes', id, { activo: false });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <Btn icon={Plus} onClick={() => setNewPlan(true)}>
          Nuevo plan
        </Btn>
      </div>
          ...
          <Card key={p.id}>
            ...
            <div className="space-y-1.5 text-sm" style={{ color: T.textSoft }}>
              <div className="flex items-center gap-2">
                ...
                {p.almuerzos} almuerzos
              </div>

              <div className="flex items-center gap-2">
                <Check size={14} style={{ color: T.green }} />
                Vigencia {p.dias} días
              </div>

              <div className="flex items-center gap-2">
                <Check size={14} style={{ color: T.green }} />
                {p.activo ? 'Plan activo' : 'Plan pausado'}
              </div>
            </div>
          </Card>
        ))}
