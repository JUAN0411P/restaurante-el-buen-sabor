...
export function SuscriptorPanel({ activeTab, user, menu, planes, suscriptores, orders, events, refresh }) {
  ...
  // Ratio asistencia: días únicos que asistió el suscriptor vs total suscritos activos
  const totalSuscritos = suscriptores.filter(s => s.activo && s.plan_id).length;
  // Días únicos donde este suscriptor asistió (estado entregado)
  const diasAsistidos = new Set(
    orders
      .filter(o => o.suscriptor_id === sub.id && o.tipo === 'suscripcion' && o.estado === 'entregado')
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.6fr_1fr] gap-4">
            ...
            <Card padding={22}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                />
                {/* Ratio asistencia */}
                <div style={{ padding: 14, background: T.terraSoft || T.bgSoft, borderRadius: 12 }}>
                  <div style={{ fontSize: 10, color: T.terracotta, ...FontMono, letterSpacing: '.1em', fontWeight: 600 }}>
                    ASISTENCIA
                  </div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginTop: 4 }}>
                    <span style={{ ...FontFraunces, fontSize: 30, color: T.terracotta, lineHeight: 1, fontStyle: 'italic' }}>
                      {diasAsistidos}/{totalSuscritos}
                    </span>
                  </div>
