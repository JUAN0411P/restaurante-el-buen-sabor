...
export function NotificationsPanel({ open, onClose, notifications, refresh, onAction, canApprove = false }) {
  ...
  const marcarLeida = async (id) => {
    await db.update('rest:notifications', id, { leida: true });
    refresh();
  };
  ...
  const marcarTodasLeidas = async () => {
    await Promise.all(
      notifications
        .filter(n => !n.leida)
        .map(n => db.update('rest:notifications', n.id, { leida: true }))
    );
    refresh();
  };

  // ✅ CORREGIDO (si usas supabase directo)
  const eliminar = async (id) => {
    const { error } = await supabase.from('notifications').delete().eq('id', id);
    if (error) console.error('eliminar notif:', error.message);
    refresh();
  };
        ...
        <>
          {unread > 0 && (
            <div className="mb-3 flex justify-end">
              <Btn size="sm" variant="ghost" onClick={marcarTodasLeidas}>
                Marcar todas leídas
              </Btn>
            </div>
          )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {yaProcesada && (
                          <Tag color={n.procesadaResultado === 'aprobada' ? 'green' : 'red'}>
...
function ExtensionActions({ notif, refresh }) {
  ...
  const aprobar = async () => {
    ...
    const sub = (await db.get('rest:subs', [])).find(s => s.id === notif.suscriptor_id);

    if (sub) {
      const venc = sub.fecha_vencimiento ? new Date(sub.fecha_vencimiento) : new Date();
      venc.setDate(venc.getDate() + notif.dias);

      await db.update('rest:subs', sub.id, {
        fecha_vencimiento: venc.toISOString().slice(0, 10),
        almuerzos_restantes: sub.almuerzos_restantes + notif.dias,
        dias_extra_compensados: (sub.dias_extra_compensados || 0) + notif.dias,
      });
    }
    ...
    await crearNotificacion({
      ...
      mensaje: `El admin aprobó ${notif.dias} día(s) adicionales.`,
